...
- (void) setupEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    AudioStreamBasicDescription inAudioStreamBasicDescription = *CMAudioFormatDescriptionGetStreamBasicDescription((CMAudioFormatDescriptionRef)CMSampleBufferGetFormatDescription(sampleBuffer)); // 输入参数
    
    AudioStreamBasicDescription outAudioStreamBasicDescription = {0}; // 初始化输出流的结构体描述为0. 很重要。{0}这种写法初始化结构体是让结构体所代表的输出符清零
    ...
    outAudioStreamBasicDescription.mFormatFlags = kMPEG4Object_AAC_LC; // 无损编码 ，0表示没有（没有子格式）
    outAudioStreamBasicDescription.mBytesPerPacket = 0; // 每一个packet的音频数据大小。如果的动态大小，设置为0。即包里的字节数是变化的可变包，动态大小的格式，需要用AudioStreamPacketDescription 来确定每个packet的大小。
...
- (NSData*) adtsDataForPacketLength:(NSUInteger)packetLength {
    ...
    /* Audio Data Transport Stream 相当于frameheader，告诉编码器一些有用的信息，如采样率，声道数，帧长度*/
